...
static QVector<ModbusCommandDef> buildCommandTable()
{
    ...
    add(0x01, "读线圈",                       "基本读写", "起始地址(2B)+数量(2B,1-2000)",           "字节数(1B)+线圈状态(nB)");
    add(0x02, "读离散输入",                   "基本读写", "起始地址(2B)+数量(2B,1-2000)",           "字节数(1B)+输入状态(nB)");
    add(0x03, "读保持寄存器",                 "基本读写", "起始地址(2B)+数量(2B,1-125)",            "字节数(1B)+寄存器值(nB)");
    add(0x04, "读输入寄存器",                 "基本读写", "起始地址(2B)+数量(2B,1-125)",            "字节数(1B)+寄存器值(nB)");
    add(0x05, "写单个线圈",                   "基本读写", "输出地址(2B)+输出值(2B:FF00/0000)",      "回显请求");
    add(0x06, "写单个寄存器",                 "基本读写", "寄存器地址(2B)+寄存器值(2B)",            "回显请求");
    add(0x0F, "写多个线圈",                   "基本读写", "起始地址(2B)+数量(2B)+字节数(1B)+值(nB)", "起始地址(2B)+数量(2B)");
    add(0x10, "写多个寄存器",                 "基本读写", "起始地址(2B)+数量(2B)+字节数(1B)+值(nB)", "起始地址(2B)+数量(2B)");

    add(0x07, "读异常状态",                   "诊断状态", "无",                                    "异常状态(1B)");
    add(0x08, "诊断",                         "诊断状态", "子功能(2B)+数据(nB)",                    "子功能(2B)+数据(nB)");
    add(0x0B, "读通信事件计数",               "诊断状态", "无",                                    "状态(2B)+事件计数(2B)");
    add(0x0C, "读通信事件日志",               "诊断状态", "无",                                    "状态(2B)+事件计数(2B)+消息计数(2B)+事件(nB)");

    add(0x14, "读文件记录",                   "文件记录", "字节数(1B)+引用组(n×7B)",                "文件响应数据");
    add(0x15, "写文件记录",                   "文件记录", "完整引用组数据",                          "回显请求");

    add(0x11, "报告从站ID",                   "其他",     "无",                                    "服务器ID数据");
    add(0x17, "读写多个寄存器",               "其他",     "读地址(2B)+读数量(2B)+写地址(2B)+写数量(2B)+写字节数(1B)+写数据(nB)", "读字节数(1B)+读数据(nB)");
    add(0x2B, "封装接口传输",                 "其他",     "MEI类型(1B)+数据(nB)",                    "MEI类型(1B)+数据(nB)");
